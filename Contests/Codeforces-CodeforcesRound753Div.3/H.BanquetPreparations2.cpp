/**
 * Author: Daniel
 * Created Time: 2021-12-05 15:43:00
**/
...
int main() {
  ...
  while (T--) {
    ...
    for (int i = 0; i < n; i++) {
      cin >> a[i] >> b[i] >> m[i];
      mp[a[i] + b[i] - m[i]].EB(i);
      mina[i] = max(0, a[i] - m[i]);
      maxa[i] = a[i] - max(0, m[i] - b[i]);
    }
    int res = SZ(mp);
    VI A(n);
    for (auto &[s, order] : mp) {
      sort(ALL(order), [&](int i, int j) {
        if (maxa[i] != maxa[j]) {
          return maxa[i] < maxa[j];
        }
        return mina[i] > mina[j];
      });
      int cnt = 0, cur = -0x3f3f3f3f;
      for (auto &i : order) {
        if (mina[i] > cur) {
          cnt++;
          cur = maxa[i];
