/**
 * Author: Daniel
 * Created Time: 2021-06-19 20:21:55
**/
...
int main()
{
    ...
    {
        if (!st[i]) continue;
        for (int j = 1; j <= SZ(fac[i]); j ++ )
        {
            int need = j;
            function<void(LL, int, int)> dfs = [&](LL cur, int u, int have)
            {
                if (need - have > SZ(fac[i]) - u) return;
                if (have >= need)
                {
                    if (need & 1) res += (y - i) / cur, res -= (y - i) / i;
                    else res -= (y - i) / cur, res += (y - i) / i;
                    return;
                }
                dfs(cur, u + 1, have);
                dfs(cur * fac[i][u], u + 1, have + 1);
            };
            dfs(1LL, 0, 0);
        }
    }
