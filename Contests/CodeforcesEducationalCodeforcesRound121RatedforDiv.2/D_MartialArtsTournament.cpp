/**
 * Author: Daniel
 * Created Time: 2022-01-19 14:33:02
**/

// time-limit: 2000
// problem-url: https://codeforces.com/contest/1626/problem/D
#include <bits/stdc++.h>
...
int main() {
  ...
  while (T--) {
    ...
    int res = (int) 2e9;
    for (int x = 1; x < (int) 1e8; x <<= 1) {
      for (int y = 1; y < (int) 1e8; y <<= 1) {
        int p = (int) (UB(ALL(s), x) - s.begin());
        if (p == n) {
          assert(x >= s.back());
          res = min(res, 2 + x - s.back());
          continue;
        }
        if (p == 0) {
          continue;
        }
        assert(p - 1 >= 0 && x >= s[p - 1]);
        int cur = x - s[p - 1];
        int l = p, r = n;
        while (l < r) {
          int mid = (l + r) >> 1;
        ...
        if (q == n) {
          assert(y >= s.back() - s[p - 1]);
          cur += y - (s.back() - s[p - 1]);
          cur += 1;
          res = min(res, cur);
          continue;
        }
        assert(y >= s[q - 1] - s[p - 1]);
        cur += y - (s[q - 1] - s[p - 1]);
        int z = 1, left = s.back() - s[q - 1];
        while (z < left) {
          z <<= 1;
        }
        cur += z - left;
