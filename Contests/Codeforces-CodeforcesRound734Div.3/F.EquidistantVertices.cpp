/**
 * Author: Daniel
 * Created Time: 2021-09-21 01:01:33
**/
...
int main()
{
    ...
    while (T -- )
    {
        ...
        auto solve = [&](int r) -> Mint
        {
            int m = SZ(g[r]);
            VE<VI> cnt(m, VI(N));
            function<void(int, int, int, int)> dfs = [&](int id, int u, int fa, int depth) -> void
            {
                cnt[id][depth] ++ ;
                for (auto &v : g[u])
                    if (v != fa)
            ...
            for (int i = 0; i < SZ(g[r]); i ++ ) dfs(i, g[r][i], r, 1);
            Mint res = 0;
            for (int d = 0; d < N; d ++ )
            {
                VE<VE<Mint>> f(m + 1, VE<Mint>(k + 1));
                f[0][0] = 1;
                for (int i = 0; i < m; i ++ )
                    for (int j = 0; j <= k; j ++ )
                    {
                        ...
                        f[i + 1][j] += f[i][j];
                        // choose
                        if (j + 1 <= k) f[i + 1][j + 1] += f[i][j] * cnt[i][d];
                    }
                res += f[m][k];
            }
            return res;
        };
        for (int r = 0; r < n; r ++ ) res += solve(r);
        cout << res << '\n';
    }
    return 0;
}
