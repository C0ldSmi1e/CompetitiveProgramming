/**
 * Author: Daniel
 * Created Time: 2021-12-11 21:56:15
**/
...
int main() {
  ...
  map<PA<int, LL>, LL> mp;
  function<LL(int, LL)> dfs = [&](int i, LL x) -> LL {
    if (i == 0) {
      return x;
    }
    if ((int) mp.count(MP(i, x)) != 0) {
      return mp[MP(i, x)];
    }
    LL res = min(x / a[i] + dfs(i - 1, x % a[i]), (x + a[i] - 1) / a[i] + dfs(i - 1, a[i] - x % a[i]));
    mp[MP(i, x)] = res;
    return res;
  };
  cout << dfs(n - 1, x) << '\n';
