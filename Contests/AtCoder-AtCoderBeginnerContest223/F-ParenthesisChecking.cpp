/**
 * Author: Daniel
 * Created Time: 2021-10-27 21:30:15
**/
...
void pushup(int u)
{
    ...
    tr[u].mn = min(tr[lson].mn, tr[lson].sum + tr[rson].mn);
}

void build(int u, int l, int r)
{
    tr[u] = {l, r, 0, INF};
    if (l == r) tr[u].sum = tr[u].mn = a[r];
    else
    {
        int mid = (l + r) >> 1;
        build(lson, l, mid);
...
{
    assert(l == r);
    if (tr[u].l >= l && tr[u].r <= r) tr[u].sum = tr[u].mn = val;
    else
    {
...
Node query(int u, int l, int r)
{
    ...
    else
    {
        ...
        else res = merge(query(lson, l, r), query(rson, l, r));
        return res;
    }
}
...
int main()
{
    ...
    while (m -- )
    {
        ...
        else
        {
            ...
            if (res.mn == 0 && res.sum == 0) cout << "Yes\n";
            else cout << "No\n";
        }
    }
    return 0;
