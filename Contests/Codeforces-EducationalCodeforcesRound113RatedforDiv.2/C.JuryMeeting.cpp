/**
 * Author: Daniel
 * Created Time: 2021-09-09 23:53:45
**/

// time-limit: 2000
// problem-url: https://codeforces.com/contest/1569/problem/C
#include <bits/stdc++.h>
...
int main()
{
    ...
    while (T -- )
    {
        ...
        int mx = a.back(), mxx = mx;
        int cntx = (int) count(ALL(a), mx);
        for (int i = n - 1; i >= 0; i -- )
            if (mxx > a[i])
            {
                mxx = a[i];
        ...
        if (cntx == 1)
        {
           ...
           int cntxx = (int) count(ALL(a), mxx);
            res = fact[cntx + cntxx] - fact[cntx] * fact[cntxx];
            int cur = cntx + cntxx;
            for (int i = 0; i < n; i ++ )
                if (a[i] < mxx)
                {
