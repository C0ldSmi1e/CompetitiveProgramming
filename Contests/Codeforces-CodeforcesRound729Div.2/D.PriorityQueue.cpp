/**
 * Author: Daniel
 * Created Time: 2021-07-06 01:06:29
**/
    ...
    for (int i = 0; i < n; i ++ )
    {
        ...
        for (int j = 0; j < n; j ++ )
            for (int k = 0; k <= n; k ++ )
                ...
                {
                    f[j + 1][k] += f[j][k];
                    if (a[j] == -1) f[j + 1][max(k - 1, 0)] += f[j][k];
                    else 
                    {
                        if (a[j] <= a[i]) f[j + 1][k + 1] += f[j][k];
                        else f[j + 1][k] += f[j][k];
                    }
                }
                else if (j == i) f[j + 1][k] += f[j][k];
                else
                {
                    f[j + 1][k] += f[j][k];
                    if (a[j] == -1)
                    {
                        if (k) f[j + 1][k - 1] += f[j][k];
                    }
                    else
                    {
                        if (a[j] < a[i]) f[j + 1][k + 1] += f[j][k];
        ...
        for (int k = 0; k <= n; k ++ ) res += f[n][k] * a[i];
    }
    cout << res << '\n';
    return 0;
}
