/**
 * Author: Daniel
 * Created Time: 2021-12-11 22:47:36
**/
...
int main() {
  ...
  while (T--) {
    int n, x, y;
    cin >> n >> x >> y;
    if (abs(x - y) > 1 || max(x, y) > (n - 1) / 2 || (x == y && 2 * (x + 1) > n)) {
      cout << "-1\n";
      continue;
    }
    if (x + y == 0) {
    ...
    for (int i = 1, xx = 0, yy = 0; i < n; i++) {
      xx += (int) (res[i] > res[i - 1] && res[i] > res[i + 1]);
      yy += (int) (res[i] < res[i - 1] && res[i] < res[i + 1]);
      if (xx == x && yy == y) {
