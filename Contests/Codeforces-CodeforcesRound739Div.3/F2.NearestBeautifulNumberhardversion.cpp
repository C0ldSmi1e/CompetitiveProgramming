/**
 * Author: Daniel
 * Created Time: 2021-09-21 23:37:19
**/
...
int main()
{
    ...
    while (T -- )
    {
        ...
        for (auto &c : s)
            v = v * 10 + (int) (c - '0');
        function<string(string, int, int)> dfs = [&](string cur, int len, int m) -> string
        {
            if (m > k || cur < s.substr(0, len)) return "";
            LL val = 0;
            for (auto &c : cur)
                val = val * 10 + (int) (c - '0');
            if (val >= v) return cur;
            for (char c = '0'; c <= '9'; c ++ )
            {
                string t = dfs(cur + c, len + 1, m + !count(ALL(cur), c));
                if (SZ(t) > 0) return t;
            }
            return "";
        };
