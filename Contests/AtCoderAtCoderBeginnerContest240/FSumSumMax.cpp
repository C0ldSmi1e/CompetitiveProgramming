/**
 * Author: Daniel
 * Created Time: 2022-03-07 13:40:46
**/
...
int main() {
  ...
  while (T--) {
    ...
    for (int i = 0; i < n; i++) {
      // beginning
      LL begB = curB + x[i];
      res = max(res, curA + begB);
      // ending
      LL endB = curB + x[i] * y[i];
      // middle
      if (begB >= 0 && endB <= 0 && x[i] != 0) {
        assert(x[i] < 0);
        LL t = -x[i];
        LL cnt = curB / t;
        res = max(res, curA + curB * cnt - t * (1 + cnt) * cnt / 2);
      }
      curB = endB;
      curA += (begB + endB) * y[i] / 2;
      res = max(res, curA);
    }
    cout << res << '\n';
  }
