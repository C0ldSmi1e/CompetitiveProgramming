/**
 * Author: Daniel
 * Created Time: 2021-09-25 00:51:24
**/

// time-limit: 4000
// problem-url: https://codeforces.com/contest/1553/problem/F
#include <bits/stdc++.h>
...
int main()
{
    ...
    for (int i = 1; i <= n; i ++ )
    {
        ...
        for (int j = x; j <= N; j += x)
        {
            int l = j, r = min(N, j + x - 1);
            res -= (fen2.get(r) - fen2.get(l - 1 >= 0 ? l - 1 : 0)) * j;
            fen1.add(l, -x);
        }
        fen2.add(x, 1);
        cout << res << " \n"[i == n];
