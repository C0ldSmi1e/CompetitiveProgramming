/**
 * Author: Daniel
 * Created Time: 2022-01-15 17:14:30
**/
...
int main() {
  ...
  VE<VL> f(n + 1, VL(k + 1, INF));
  f[0][0] = 0;
  for (int i = 0; i < n; i++) {
    for (int j = 0; j <= k; j++) {
      f[i + 1][j] = min(f[i + 1][j], f[i][j] + a[i] * (d[i + 1] - d[i]));
      if (j == k || i == 0) {
        continue;
      }
      for (int l = j + 1, nxt = i + 1; l <= k && nxt <= n; l++, nxt++) {
        f[nxt][l] = min(f[nxt][l], f[i][j] + a[i - 1] * (d[nxt] - d[i]));
      }
    }
  }
  cout << MIN(ALL(f[n])) << '\n';
