/**
 * Author: Daniel
 * Created Time: 2021-11-29 10:28:43
**/

// time-limit: 2000
// problem-url: https://codeforces.com/contest/1611/problem/F
#include <bits/stdc++.h>
...
int main() {
  ...
  while (T--) {
    ...
    //debug(pre);
    //Fenwick<LL> fen(pre, [&](LL i, LL j) { return min(i, j); });
    Rmq<LL> fen(pre, [&](LL i, LL j) -> LL { return min(i, j); });
    int x = -1, y = -1;
    auto check = [&](int len) -> bool {
      //debug(len);
      for (int i = len - 1; i < n; i++) {
        LL mn = fen.get(i - len + 1, i);
        LL t = (i - len >= 0 ? pre[i - len] : 0);
        //debug(i, mn, t);
        if (s + mn - t >= 0) {
          x = i - len + 1;
          y = i;
