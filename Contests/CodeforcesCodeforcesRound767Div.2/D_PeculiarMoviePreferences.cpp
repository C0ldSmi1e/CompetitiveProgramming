/**
 * Author: Daniel
 * Created Time: 2022-01-22 23:13:10
**/

// time-limit: 2000
// problem-url: https://codeforces.com/contest/1629/problem/D
#include <bits/stdc++.h>
...
int main() {
  ...
  while (T--) {
    ...
    for (int i = 0; i < n; i++) {
      if (SZ(g[i]) == 3) {
        string t = g[i].substr(1);
        flag |= (int) (S.count(t) > 0);
      } else if (SZ(g[i]) == 2) {
        string t = g[i];
        reverse(ALL(t));
        S.IS(t);
