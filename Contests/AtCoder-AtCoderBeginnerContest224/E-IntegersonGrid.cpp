/**
 * Author: Daniel
 * Created Time: 2021-10-26 15:04:04
**/
...
int main()
{
    ...
    VI dist(id, -1), res(id, -1);
    VI row(n, -1), col(m, -1);
    for (int v = mm - 1; v >= 0; v -- )
    {
        for (auto &[x, y] : pos[v])
        {
            int t = mp[MP(x, y)];
            dist[t] = max(dist[t], row[x] + 1);
            dist[t] = max(dist[t], col[y] + 1);
            res[t] = dist[t];
        }
        for (auto &[x, y] : pos[v])
        {
            int t = mp[MP(x, y)];
            row[x] = max(row[x], dist[t]);
            col[y] = max(col[y], dist[t]);
        }
    }
    for (auto &u : res)
        cout << u << '\n';
