/**
 * Author: C0ldSmi1e
 * Created Time: 01/24/2024 07:51:58 AM
**/

// time-limit: 1000
// problem-url: https://www.codechef.com/START118B/problems/SUBCOUNT
#include <bits/stdc++.h>
...
int main() {
  ...
  while (T--) {
    ...
    string st = s + t;
    string ts = t + s;
    StringHash st_hash(st);
    StringHash ts_hash(ts);
    int cs = 1;
    int ct = (int) (s == t);
    int cst = 0;
    int cts = 0;
    for (int i = 1; i < n; i++) {
      if (st_hash.Get(0, n) == st_hash.Get(i, i + n)) {
        cst += 1;
      }
      if (ts_hash.Get(0, n) == ts_hash.Get(i, i + n)) {
        cts += 1;
      }
    }
    Mint cnt_s = 0;
