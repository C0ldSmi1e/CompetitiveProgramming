/**
 * Author: Daniel
 * Created Time: 2022-03-06 13:00:31
**/

// time-limit: 2000
// problem-url: https://codeforces.com/contest/1644/problem/C
#include <bits/stdc++.h>
...
int main() {
  ...
  while (T--) {
    ...
    for (int len = 0; len <= n; len++) {
      for (int i = len; i <= n; i++) {
        sum[len] = max(sum[len], pre[i] - pre[i - len]);
        mx = max(mx, sum[len]);
      }
    }
    VI res(n + 1);
    for (int len = 0; len <= n; len++) {
      res[len] = mx;
      for (int i = len; i <= n; i++) {
        res[len] = max(res[len], sum[i] + len * x);
      }
      mx = max(mx, res[len]);
    }
    for (int i = 0; i <= n; i++) {
