/**
 * Author: Daniel
 * Created Time: 2022-01-30 12:29:32
**/

// time-limit: 2000
// problem-url: https://codeforces.com/contest/1631/problem/E
#include <bits/stdc++.h>
...
int main() {
  ...
  VPII seg;
  seg.EB(l[a[0]], r[a[0]]);
  int remove = 1 + (int) (0 < r[a[0]]);
  while (seg.back().S < n - 1) {
    int rst = -1;
    for (int i = seg.back().F; i <= seg.back().S; i++) {
      rst = max(rst, r[a[i]]);
    }
    if (rst > seg.back().S) {
      seg.EB(l[a[rst]], r[a[rst]]);
      remove += 1;
    } else {
      int i = seg.back().S + 1;
      seg.EB(l[a[i]], r[a[i]]);
      remove += 1 + (int) (i < r[a[i]]);
    }
  }
  cout << n - remove << '\n';
  return 0;
