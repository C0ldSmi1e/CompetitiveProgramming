/**
 * Author: Daniel
 * Created Time: 2022-01-06 14:33:47
**/
...
int main() {
  ...
  int a, b, c;
  cin >> a >> b >> c;
  VE<VE<VD>> f(100, VE<VD>(100, VD(100, 1.0)));
  for (int i = 99; i >= a; i--) {
    for (int j = 99; j >= b; j--) {
      for (int k = 99; k >= c; k--) {
        if (i > 0) {
          f[i - 1][j][k] += f[i][j][k] * ((double) (i - 1) / (i - 1 + j + k));
        }
        if (j > 0) {
          f[i][j - 1][k] += f[i][j][k] * ((double) (j - 1) / (i - 1 + j + k));
        }
        if (k > 0) {
          f[i][j][k - 1] += f[i][j][k] * ((double) (k - 1) / (i - 1 + j + k));
        }
      }
    }
  }
