/**
 * Author: Daniel
 * Created Time: 2022-03-09 13:54:58
**/
...
int main() {
  ...
  while (T--) {
    ...
    string s;
    cin >> n >> s;
    VE<VE<Mint>> f(2, VE<Mint>(26));
    int nn = (n + 1) / 2;
    for (int i = 0; i < nn; i++) {
      VE<VE<Mint>> nf(2, VE<Mint>(26));
      if (i == 0) {
        for (int c = 0; c <= (int) (s[i] - 'A'); c++) {
          nf[i == 0 && (int) (s[i] - 'A') == c][c] = 1;
        }
      }
      int l = (int) (s[i] - 'A');
      for (int t = 0; t <= 1; t++) {
        for (int c = 0; c < 26; c++) {
          for (int x = 0; x <= (t == 1 ? l : 25); x++) {
            nf[t && x == l][x] += f[t][c];
          }
        }
      }
    ...
    Mint res = 0;
    bool flag = true;
    for (int i = nn - 1, j = (n % 2 ? nn - 1 : nn); i >= 0; i--, j++) {
      if (s[i] == s[j]) {
        continue;
      }
      if (s[i] > s[j]) {
