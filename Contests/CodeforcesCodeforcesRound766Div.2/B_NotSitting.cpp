/**
 * Author: Daniel
 * Created Time: 2022-01-17 18:57:38
**/
...
int main() {
  ...
  while (T--) {
    ...
    if (n % 2 == 1 && m % 2 == 1) {
      q.EB(n / 2, m / 2);
      for (int i = 0; i < 4; i++) {
        d[n / 2][m / 2] = max(d[n / 2][m / 2], Get(n / 2, m / 2, cx[i], cy[i]));
      }
    } else if (n % 2 == 1) {
      q.EB(n / 2, m / 2);
      q.EB(n / 2, m / 2 - 1);
    ...
    for (int qq = 0; qq < SZ(q); qq++) {
      ...
      for (int i = 0; i < 4; i++) {
        ...
        for (int j = 0; j < 4; j++) {
          d[tx][ty] = max(d[tx][ty], Get(tx, ty, cx[j], cy[j]));
        }
        q.EB(tx, ty);
      }
    }
