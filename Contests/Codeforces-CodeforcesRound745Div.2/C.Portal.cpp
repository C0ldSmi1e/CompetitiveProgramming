/**
 * Author: Daniel
 * Created Time: 2021-10-24 16:54:35
**/

// time-limit: 1000
// problem-url: https://codeforces.com/contest/1581/problem/C
#include <bits/stdc++.h>
...
int main()
{
    ...
    while (T -- )
    {
        ...
        auto get = [&](int x1, int y1, int x2, int y2) -> int
        {
            ...
            assert(x1 <= x2 && y1 <= y2);
            return g[x2][y2] + g[x1 - 1][y1 - 1] - g[x2][y1 - 1] - g[x1 - 1][y2];
        };
        int res = (int) 2e9;
        for (int x1 = 1; x1 <= n; x1 ++ )
            for (int x2 = x1 + 4; x2 <= n; x2 ++ )
            {
                ...
                for (int y = 1; y <= m; y ++ )
                {
                    f[y] = (x2 - x1 - 1) - get(x1 + 1, y, x2 - 1, y);
                    s[y] = f[y] + (2 * (y - 1) - get(x1, 1, x1, y - 1) - get(x2, 1, x2, y - 1)) + get(x1 + 1, 2, x2 - 1, y - 1);
                }
                for (int y = m - 1; y >= 1; y -- )
                    s[y] = min(s[y], s[y + 1]);
                for (int y = 1; y + 3 <= m; y ++ )
                {
                    int cur = s[y + 3] + f[y];
                    cur -= 2 * y - get(x1, 1, x1, y) - get(x2, 1, x2, y);
                    cur -= get(x1 + 1, 2, x2 - 1, y);
                    res = min(res, cur);
                }
            }
        cout << res << '\n';
