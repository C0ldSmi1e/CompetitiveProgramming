/**
 * Author: Daniel
 * Created Time: 2022-01-05 17:36:53
**/

// time-limit: 2000
// problem-url: https://atcoder.jp/contests/abc208/tasks/abc208_e
#include <bits/stdc++.h>
...
int main() {
  ...
  int m = SZ(digit);
  VE<VE<VE<map<LL, LL>>>> f(m, VE<VE<map<LL, LL>>>(2, VE<map<LL, LL>>(2)));
  function<LL(int, int, int, LL)> dp = [&](int n, int limit, int zero, LL x) -> LL {
    if (n == -1) {
      return (LL) (x <= k && zero == 0);
    }
    if (f[n][limit][zero].count(x)) {
      return f[n][limit][zero][x];
    }
    LL res = 0;
    int mx = (limit ? digit[n] : 9);
    for (int y = 0; y <= mx; y++) {
      res += dp(n - 1, limit && mx == y, zero && y == 0, zero ? y : x * y);
    }
    f[n][limit][zero][x] = res;
    return res;
  };
  cout << dp(m - 1, 1, 1, 1) << '\n';
  return 0;
