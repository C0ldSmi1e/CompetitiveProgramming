/**
 * Author: Daniel
 * Created Time: 2021-07-25 11:43:19
**/
...
int main()
{
    ...
    for (int i = 0; i < n; i ++ )
    {
        ...
        if (i > 0)
        {
            if (s[i] == 'h') f[i][1] += f[i - 1][0];
            if (s[i] == 'o') f[i][2] += f[i - 1][1];
            if (s[i] == 'k') f[i][3] += f[i - 1][2];
            if (s[i] == 'u') f[i][4] += f[i - 1][3];
            if (s[i] == 'd') f[i][5] += f[i - 1][4];
            if (s[i] == 'a') f[i][6] += f[i - 1][5];
            if (s[i] == 'i') f[i][7] += f[i - 1][6];
        }
    }
    cout << f.back().back() << '\n';
    return 0;
