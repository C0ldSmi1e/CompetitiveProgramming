/**
 * Author: Daniel
 * Created Time: 2021-06-19 13:20:35
**/
...
int main()
{
    ...
    while (T -- )
    {
        ...
        cin >> n >> m >> x >> y;
        x -- , y -- ;
        VPII a{{0, 0}, {0, m - 1}, {n - 1, m - 1}, {n - 1, 0}};
        int x1 = -1, y1 = -1, x2 = -1, y2 = -1;
        LL mx = -1;
        for (int i = 0; i < 4; i ++ )
            for (int j = 0; j < 4; j ++ )
            {
                LL sum = abs(a[i].F - x) + abs(a[i].S - y);
                sum += abs(a[j].F - a[i].F) + abs(a[j].S - a[i].S);
                sum += abs(a[j].F - x) + abs(a[j].S - y);
                if (sum > mx)
                {
                    mx = sum;
                    x1 = a[i].F;
