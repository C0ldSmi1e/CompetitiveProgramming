/**
 * Author: Daniel
 * Created Time: 2021-08-10 13:15:52
**/
...
int main()
{
    ...
    sort(ALL(row));
    sort(ALL(col));
    row.ER(unique(ALL(row)), row.end());
    col.ER(unique(ALL(col)), col.end());
    for (auto &[x, y] : a)
    {
        x = (int) (LB(ALL(row), x) - row.begin()) + 1;
        y = (int) (LB(ALL(col), y) - col.begin()) + 1;
        cout << x << ' ' << y << '\n';
    }
    return 0;
}
