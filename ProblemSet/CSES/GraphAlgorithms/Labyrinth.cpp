/**
 * Author: Daniel
 * Created Time: 2021-06-14 09:33:35
**/
...
int main()
{
    ...
    VE<VPII> pre(n, VPII(m, {-1, -1}));
    int dx[4] = {1, -1, 0, 0};
    int dy[4] = {0, 0, 1, -1};
    char dir[4] = {'D', 'U', 'R', 'L'};
    VE<VE<char>> go(n, VE<char>(m));
    VPII q{st};
    for (int qq = 0; qq < SZ(q); qq ++ )
    {
        ...
        for (int i = 0; i < 4; i ++ )
        {
            ...
            if (g[tx][ty] == '#' || pre[tx][ty].F != -1) continue;
            pre[tx][ty] = {x, y};
            go[tx][ty] = dir[i];
            q.EB(tx, ty);
        }
    }
    if (pre[ed.F][ed.S].F == -1) cout << "NO\n";
    else
    {
        string path;
        auto [x, y] = ed;
        while (x != st.F || y != st.S)
        {
            path += go[x][y];
            int tx = x, ty = y;
            if (x == st.F && y == st.S) break;
            x = pre[tx][ty].F, y = pre[tx][ty].S;
        }
        reverse(ALL(path));
        cout << "YES\n";
        cout << SZ(path) << '\n';
