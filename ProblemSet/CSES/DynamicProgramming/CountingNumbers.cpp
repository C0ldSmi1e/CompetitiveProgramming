/**
 * Author: Daniel
 * Created Time: 2021-07-06 19:29:47
**/
...
int main()
{
    ...
    auto dp = [&](LL x) -> LL
    {
        ...
        for (int i = 0; i < n; i ++ )
        {
            int x = nums[i], left = n - i - 1;
            int same = (int) (i > 0 && nums[i - 1] <= x - 1);
            res += (LL) (x - same - !i) * qp(9, left);
            if (i > 0 && x == nums[i - 1]) break;
            if (i == n - 1) res ++ ;
        }
        for (int i = 0; i < n; i ++ ) res += qp(9, n - i - 1);
        return res;
    };
    LL l, r;
    cin >> l >> r;
