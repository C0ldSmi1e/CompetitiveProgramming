/**
 * Author: Daniel
 * Created Time: 2021-07-07 15:20:41
**/
...
int main()
{
    ...
    for (int i = 0; i < n; i ++ )
        for (int j = 0; j < i; j ++ )
            mp[a[i].F + a[j].F].IS(MP(-a[j].S, -a[i].S));
    debug(mp);
    for (int i = 0; i < n; i ++ )
        for (int j = 0; j < i; j ++ )
        {
            LL need = x - a[i].F - a[j].F;
            if (need < 1) continue;
            if (SZ(mp[need]))
            {
                auto it = mp[need].begin();
                int x = (*it).F, y = (*it).S;
                set<int> S{-x, -y, a[i].S, a[j].S};
                if (SZ(S) != 4) continue;
                cout << a[i].S + 1 << ' ' << a[j].S + 1 << ' ';
                cout << -x + 1 << ' ' << -y + 1 << '\n';
                return 0;
            }
            mp[a[i].F + a[j].F].ER(MP(-a[j].S, -a[i].S));
        }
    cout << "IMPOSSIBLE\n";
    return 0;
}
