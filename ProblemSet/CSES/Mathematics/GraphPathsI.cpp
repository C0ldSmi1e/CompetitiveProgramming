/**
 * Author: Daniel
 * Created Time: 2021-12-02 18:22:51
**/
...
int main() {
  ...
  auto mul = [&](VE<VE<Mint>> &a, VE<VE<Mint>> &b, VE<VE<Mint>> &c) -> void {
    // a = b * c
    VE<VE<Mint>> t(n, VE<Mint>(n));
    for (int k = 0; k < n; k++) {
      for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
          t[i][j] += b[i][k] * c[k][j];
