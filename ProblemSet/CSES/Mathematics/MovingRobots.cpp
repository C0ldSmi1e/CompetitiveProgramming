/**
 * Author: Daniel
 * Created Time: 2021-12-03 12:55:36
**/
...
int main() {
  ...
  VE<VE<LD>> res(n, VE<LD>(n, 1.0));
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      VE<VE<VE<LD>>> f(n, VE<VE<LD>>(n, VE<LD>(steps + 1)));
      f[i][j][0] = 1.0;
        ...
        for (int x = 0; x < n; x++) {
          for (int y = 0; y < n; y++) {
            ...
            for (int k = 0; k < 4; k++) {
              int tx = dx[k] + x;
              int ty = dy[k] + y;
              cnt += (int) (tx >= 0 && ty >= 0 && tx < n && ty < n);
            }
            for (int k = 0; k < 4; k++) {
              int tx = dx[k] + x;
              int ty = dy[k] + y;
              if (tx >= 0 && ty >= 0 && tx < n && ty < n) {
                f[tx][ty][it + 1] += f[x][y][it] / cnt;
              }
            }
          }
        }
