/**
 * Author: Daniel
 * Created Time: 2021-06-16 13:43:29
**/
...
int main()
{
    ...
    VL sum(n), dist(n);
    VI sz(n);
    function<void(int, int, LL)> get = [&](int u, int fa, LL depth)
    {
        sz[u] = 1;
        if (u == 0) sum[0] += SZ(g[u]) * depth;
        else sum[0] += (SZ(g[u]) - 1) * depth;
        for (auto &v : g[u])
            if (v != fa)
            {
                get(v, u, depth + 1);
    ...
    function<void(int, int)> dfs = [&](int u, int fa)
    {
        if (fa != -1)
        {
            sum[u] = sum[fa] - (dist[u] + sz[u]) + (n - sz[u]);
            sum[u] += dist[u];
        }
        for (auto &v : g[u])
            if (v != fa)
