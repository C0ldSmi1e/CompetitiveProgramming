/**
 * Author: Daniel
 * Created Time: 2021-07-10 14:13:23
**/
...
int main()
{
    ...
    sort(ALL(all));
    all.ER(unique(ALL(all)), all.end());
    for (auto &u : val) u = (int) (LB(ALL(all), u) - all.begin());
    VE<VI> g(n);
    for (int i = 0; i < n - 1; i ++ )
    {
        int a, b;
    ...
    {
        int x = who[i], v = val[x];
        if (last[v] != -1) fen.add(last[v], -1);
        fen.add(i, 1);
        last[v] = i;
        for (auto &[L, id] : R[i])
        {
            res[id] = fen.get(i);
            if (L > 0) res[id] -= fen.get(L - 1);
        }
    }
    for (auto &u : res)
        cout << u << ' ';
